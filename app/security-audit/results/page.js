// appsecurity-audit/results/page.js
...
export default function SecurityAuditStatsDashboard() {
  ...
  const [checklistItems, setChecklistItems] = useState([
    {
      ...
      category: "패스워드 정책",
      description:
        "암호에 대문자, 소문자, 숫자, 특수문자 중 3종류 이상을 포함해야 함",
    },
    {
      id: 8,
      name: "최대 암호 사용 기간",
      category: "패스워드 정책",
      description: "암호는 최대 90일 이내로 변경되어야 함",
    },
    {
      id: 9,
      name: "암호 기록 크기",
      category: "패스워드 정책",
      description: "최근 사용한 5개의 암호는 재사용할 수 없음",
    },
  ]);
  ...
  const prepareItemStats = () => {
    const itemStatsData = checklistItems.map((item) => {
      ...
      const passRate =
        itemLogs.length > 0 ? (passedCount / itemLogs.length) * 100 : 0;
