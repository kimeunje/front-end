...
:: === API 서버 설정 ===
set SERVER_URL="http://127.0.0.1:5002"
...
:login
    set /p USER="이름을 입력하세요: "
    set /p EMPID="사번을 입력해주세요: "

@REM set USER="김은제"
@REM set EMPID="B01052"

:: === 메인 실행 ===
:main
    @REM 검증 로직 실행
    call :authenticate_user "사용자 검증" "INFO"
    call :generate_logs "감사 로그 생성" "INFO"
    ...
    call :printer_active_check "프린터 목록 검사" "INFO"
    call :remote_computer_check "원격 설정 검사" "INFO"
    call :program_version_check "비인가 프로그램 설치 검사" "INFO"
    call :security_solution_version_check "보안솔루션 버전 검사" "INFO"
    call :window_version_check "윈도우 최신 패치 검사" "INFO"
    
    @REM 사용자 환경변수 nicednb_audit_user_id 제거
    REG delete HKCU\Environment /F /V nicednb_audit_user_id >nul 2>&1
    REG delete HKCU\Environment /F /V nicednb_audit_emp_id >nul 2>&1

    @REM 로직 종료 후 출력 창 실행
    call :exit_to_borwser
...
:: === 로그 생성 요청 ===
:generate_logs
call :log_message "감사 로그 생성 요청 시작" "INFO"
    ...
    try { ^
        ...
        -Method Post ^
        -Uri '%SERVER_URL%/api/generate_logs' ^
        -Body $body ^
        -ContentType 'application/json; charset=utf-8'; ^
        if ($response.status -eq 'already_generated') { ^
            %YELLOW% '이미 로그가 생성되었습니다.'; ^
            exit 0; ^
        } ^
    } catch { ^
    ...
    if %ERRORLEVEL% EQU 0 (
        call :log_message "로그 생성 요청 성공" "PASS"
    ) else (
        call :log_message "로그 생성 요청 실패" "FAIL"
    )
...
:authenticate_user
    ...
    powershell -Command ^
        ...
        { ^
            ...
            Set-ItemProperty ^
                -Path 'HKCU:\Environment' ^
                -Name 'nicednb_audit_emp_id' ^
                -Value $response.emp_id ^
                -Type String ^
                -Force; ^
        }; ^
        } catch { ^
...
:screen_saver_check
    ...
    if %ERRORLEVEL% EQU 0 (
        call :log_message "화면 보호기 검사 성공" "PASS"
    ) else (
        call :log_message "화면 보호기 검사 실패" "FAIL"
    )
